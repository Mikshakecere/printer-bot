...
def fetch_race_tiles(ct_id):
    tiles = fetch_ct_tiles(ct_id)
    return fetch_regulars(tiles), fetch_banners(tiles), fetch_relics(tiles)

def fetch_regulars(tiles):
    regular = []
    for tile in tiles:
